...
- **Vietnamese**, contributed by [@vuanhvu11982](https://github.com/vuanhvu11982). Around two thirds complete; the rest falls back to English and is listed in the translation editor for anyone who would like to finish it
- Improvements to **Turkish** from [@BouRock](https://github.com/BouRock), **Hungarian** from [@JohnFowler58](https://github.com/JohnFowler58) and **Italian** from [@bovirus](https://github.com/bovirus), some of which had been waiting since April, all now credited in the files they wrote
...
- **Translators are credited, and you can see who they are.** Each translation now records everyone who has worked on it rather than a single author, and Settings → Appearance names them beside the language they translated. Previously the file recorded one name and nothing ever displayed it, so no translator was credited at all — while a plugin's author had always been shown. A single name also meant everyone after the first had to erase the person before them, which is exactly what had been happening
...
- **The enhanced service selector is now the default.** It shows which service is active and lets one be swapped in place, where the classic selector only lists them. Anyone who chose the classic selector keeps it
- **Changing the extra output no longer re-translates.** Switching it between backward translation, summary and rewrite, or changing a summary length, used to throw away the translation on screen and fetch the identical text again a round trip later. Only the extra panel is recomputed now. Turning the panel on from the Options menu also fills it in, rather than revealing an empty one that stayed empty until the next translation
- Translation files no longer carry a `version` or a `last_updated` field. Both were maintained by hand and drifted, neither was ever read, and one contributor reasonably took `version` to mean the application's
...
### Fixed
- **Icons were drawn too large at any zoom above 100%.** The size was scaled twice, so at 125% they came out a quarter too big and at 150% half again, and the oversized glyph was then squeezed into a button sized correctly. Invisible at 100%, which is why it lasted; most visible on the add, edit and delete buttons beside the service preset picker
- **The dictionary service pickers show each service’s icon**, as the translator picker already did. They had always had the icon available and simply never drew it
- **The settings sidebar answers the pointer**, with a rollover that matches the selected row exactly rather than hugging the label, so short entries and indented ones no longer get differently shaped highlights. Section headings are quieter than the pages they label, spacing through the whole dialog follows one scale instead of four numbers that had drifted apart, and grouped pages sit less far in
- **Translation requests are no longer sent twice when a service fails mid-request.** A 5xx response can mean the service did the work and failed to report it, and every such response was retried regardless of the request type — so a translation could be performed, and charged for, more than once from a single attempt. Retries on server errors are now limited to requests that are safe to repeat
- **Rate-limited requests are now retried, and the wait the service asks for is obeyed.** A 429 means the request was refused rather than performed, which makes it both safe to repeat and the single response most worth repeating; it was previously the one case never retried
- **Far fewer simultaneous connections to a single service.** The underlying default allowed a hundred, which is a server’s number: a desktop application opening that many to one translation API is not faster, it is inviting a rate limit
...
- **A local AI endpoint no longer demands an API key.** Ollama and similar servers need none, but five separate checks refused to run without one, so translation, dictionary, the connection test and vision OCR all failed against a local model. Vision also reported it as an authentication failure, which was the wrong diagnosis: nothing had rejected a key, none was needed
- **Choosing English as the interface language now survives a restart.** Settings are written without their default values, so picking English stored nothing at all, which was indistinguishable from never having chosen — and every launch then re-detected the system language and overrode it. On a non-English machine the choice never held
- **The Settings dialog lays out correctly right to left.** Section rules were measured from the left edge and so were never drawn at all in Arabic; the divider in the plugins list stayed on the side it was built with rather than the side it belonged on; and hints wrapped at a fixed measure that left their text starting a full measure in from the right, reading as centred. The sidebar also now shows which pages sit under a heading
- The application no longer reports its own English strings as a missing language file on every start
